...
//#define _DEBUG
#define OSGPPU_CUDAKERNEL_LIBRARY
...
//-----------------------------------------------------------------------------
ProcessingModule* g_ProcessingModule = NULL;
...
extern "C" bool OSGPPU_CUDAK_EXPORT OSGPPU_MODULE_ENTRY(UnitInOutModule* parent) 
{
    ...
    parent->setModule(g_ProcessingModule);

    return true;
}

//-----------------------------------------------------------------------------
// Remove the module by clearing up used memory
//-----------------------------------------------------------------------------
extern "C" void OSGPPU_CUDAK_EXPORT OSGPPU_MODULE_RELEASE(void) 
{
	osg::notify(osg::INFO) << "osgPPU - Module - Remove module, clear memory!" << std::endl;

	if (g_ProcessingModule)
	{
		delete g_ProcessingModule;
		g_ProcessingModule = NULL;
	}
}
