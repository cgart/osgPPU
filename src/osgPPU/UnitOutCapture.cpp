...
namespace osgPPU
{
    ...
    UnitOutCapture::UnitOutCapture(const UnitOutCapture& unit, const osg::CopyOp& copyop) :
		...
		mUseInputTextureInternalFormat(unit.mUseInputTextureInternalFormat)
    {
    ...
    UnitOutCapture::UnitOutCapture() : UnitOut()
    {
        ...
        mExtension = "png";
        mShotOnce = false;
		mUseInputTextureInternalFormat = false;
    }
    ...
    void UnitOutCapture::captureInput(osg::State* state)
    {
        ...
        for (unsigned int i=0; i < mInputTex.size(); i++)
        {
			...
			if (mUseInputTextureInternalFormat)
			...
			osgDB::ReaderWriter::WriteResult res = osgDB::Registry::instance()->writeImage(*img, filename, osgDB::Registry::instance()->getOptions());
            if (res.success())
                osg::notify(osg::WARN) << " OK" << std::endl;
            else
                osg::notify(osg::FATAL) << " failed! (" << res.message() << ")" << std::endl;
